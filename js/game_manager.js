...
//  var start = document.getElementById("starttiles").value;
  ...
  this.startTiles     = localStorage.getItem("startingtilesinput");     //number of tiles to start with
...
GameManager.prototype.setup = function () {
  ...
  if (previousState) {
    ...
    this.score       = localStorage.getItem("startscoreinput"); //Change this value to change starting score
    this.over        = false;
    this.won         = false;
...
GameManager.prototype.addRandomTile = function () {
  if (this.grid.cellsAvailable()) {
    var value = Math.random() < 0.9 ? localStorage.getItem("startv1") : localStorage.getItem("startv2");   //numbers on tiles when spawning
    ...
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && next.value === tile.value && !next.mergedFrom) {
          var merged = new Tile(positions.next, tile.value * 2);    //number tiles are multiplied
          ...
          if (merged.value === localStorage.getItem("winscore")) self.won = true;   //score to win
        } else {
